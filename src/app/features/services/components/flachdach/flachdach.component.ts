...
export class FlachdachComponent implements OnInit {
  ...
  city?: City;
  cityKey?: string;
  serviceKey = 'flachdachpruefung';
  serviceName = 'Flachdachdichtheitsprüfung';
  breadcrumbs: BreadcrumbItem[] = [];

  faqs = [
    { question: 'Wann ist eine Flachdachprüfung sinnvoll und für wen?', answer: 'Eine Prüfung ist ratsam bei unklaren Feuchtigkeitsschäden, nach schweren Unwettern oder zur Abnahme vor Ablauf der Gewährleistung. Wir prüfen alles vom privaten Balkon und Garagendach bis hin zu großen Industriehallen, Gewerbeobjekten und Tiefgaragen.', isOpen: false },
    { question: 'Welche Verfahren werden genutzt und entstehen dabei Schäden?', answer: 'Wir setzen auf zerstörungsfreie Methoden wie das Rauchgas-, Dampferzeugungs- oder Impulsstromverfahren. Dabei wird ein Gemisch unter die Abdichtung gepumpt, um Leckagen punktgenau sichtbar zu machen. Das Dach muss nicht großflächig geöffnet werden; lediglich kleine Anschlussöffnungen werden fachgerecht erstellt und sofort wieder verschlossen.', isOpen: false },
    { question: 'Wie lange dauert die Prüfung und was erhalte ich als Ergebnis?', answer: 'Je nach Objektgröße dauert die Prüfung zwischen 2 und 6 Stunden, bei Industriehallen ggf. einen Arbeitstag. Als Ergebnis erhalten Sie ein umfassendes, bebildertes Prüfprotokoll, das als gerichtsfester Nachweis für Versicherungen oder zur gezielten Reparaturplanung dient.', isOpen: false }
  ];
  ...
  private buildBreadcrumbs(): void {
    this.breadcrumbs = [
      ...
      ...(this.city
        ? [{ label: 'Flachdachprüfung', url: '/leistungen/flachdachpruefung' }, { label: this.city.name }]
        : [{ label: 'Flachdachprüfung' }]
      )
    ];
  }
  ...
  private applySeo(): void {
    ...
    this.seoService.updateMetaTags({ title: seoTitle, description: seoDesc, url: canonicalUrl, keywords: `Flachdachdichtheitsprüfung ${cityName}, Leckortung ${cityName}, Rauchtest Flachdach ${regionName}, Flachdach prüfen, Industriehalle undicht` });
  }

  get titleWithCity(): string { return this.city ? `${this.serviceName} in ${this.city.name}` : this.serviceName; }
  get subtitleWithCity(): string { return this.city ? `Rauch- und Dampftest im Raum ${this.city.region}` : 'Mit Rauch- und Dampftest'; }
  ...
  get regionalTextFlachdach(): any {
    ...
    return {
      intro: `${this.city.name}, ${this.city.localHook}, ist ein typischer Standort für Gewerbe- und Industrieimmobilien mit Flachdach. Undichte Stellen führen hier oft zu schleichenden Schäden, die erst spät bemerkt werden.`,
