...
export class DachfensterComponent implements OnInit {
  ...
  faqs = [
    { question: 'Welche Dachfenster-Typen gibt es?', answer: 'Es gibt Schwingfenster, Klapp-Schwingfenster, Dachschiebefenster und Ausstiegsfenster. Wir beraten Sie zur passenden Lösung für Ihr Dach.', isOpen: false },
    { question: 'Wie lange dauert der Einbau eines Dachfensters?', answer: 'Ein Standard-Dachfenster wird in der Regel innerhalb eines Tages eingebaut. Bei komplexeren Projekten oder mehreren Fenstern kann es 2-3 Tage dauern.', isOpen: false },
    { question: 'Was kostet ein Dachfenster mit Einbau?', answer: 'Die Kosten variieren je nach Größe, Typ und Ausstattung. Für ein Standard-Dachfenster inkl. Einbau können Sie mit 1.500-3.500€ rechnen. Kontaktieren Sie uns für ein individuelles Angebot.', isOpen: false },
    { question: 'Sind Dachfenster förderfähig?', answer: 'Ja, der Einbau energieeffizienter Dachfenster kann über die BEG (Bundesförderung für effiziente Gebäude) mit bis zu 20% gefördert werden. Auch steuerliche Absetzung ist möglich.', isOpen: false },
    { question: 'Wie verbessern Dachfenster die Energieeffizienz?', answer: 'Moderne Dachfenster mit Dreifachverglasung reduzieren Wärmeverluste erheblich und verbessern die Dämmung. Sie erfüllen die aktuellen Anforderungen des GEG (Gebäudeenergiegesetz).', isOpen: false }
  ];
  ...
  private buildBreadcrumbs(): void {
    this.breadcrumbs = [
      ...
      ...(this.city
        ? [{ label: 'Dachfenster', url: '/leistungen/dachfenster' }, { label: this.city.name }]
        : [{ label: 'Dachfenster' }]
      )
    ];
  }
  ...
  private applySeo(): void {
    ...
    this.seoService.updateMetaTags({ title: seoTitle, description: seoDesc, url: canonicalUrl, keywords: `Dachfenster ${cityName}, Velux ${cityName}, Dachfenster Einbau ${regionName}, Dachgaube ${cityName}, Dachfenster Kosten` });
  }

  get titleWithCity(): string { return this.city ? `${this.serviceName} in ${this.city.name}` : this.serviceName; }
  get subtitleWithCity(): string { return this.city ? `Mehr Licht und Lebensqualität im Raum ${this.city.region}` : 'Mehr Licht, mehr Raum, mehr Lebensqualität'; }
  toggleFaq(index: number): void { this.faqs[index].isOpen = !this.faqs[index].isOpen; }
  toggleCard(index: number): void { this.openCard = this.openCard === index ? null : index; }
  nextSlide(): void { this.currentSlide = (this.currentSlide + 1) % this.images.length; }
  prevSlide(): void { this.currentSlide = (this.currentSlide - 1 + this.images.length) % this.images.length; }
  goToSlide(i: number): void { this.currentSlide = i; }

  get regionalTextDachfenster(): any {
    if (!this.city) return null;
    return {
      intro: `${this.city.name}, ${this.city.localHook}, bietet viele Altbestände, bei denen ein modernes Dachfenster ungenutzten Dachraum in lichtdurchfluteten Wohnraum verwandelt.`,
